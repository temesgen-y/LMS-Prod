  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
                  ...
                  <div className="max-h-72 overflow-y-auto divide-y divide-gray-50">
                    {notifs.length === 0 ? (
                      ...
                      <button key={n.id} type="button" onClick={() => { markRead(n.id); if (n.link) { setNotifOpen(false); router.push(n.link); } }}
                        className={`w-full text-left px-4 py-3 hover:bg-gray-50 ${!n.is_read ? 'bg-teal-50' : ''}`}>
              ...
              <>
                ...
                <div className="absolute right-0 top-full mt-1 py-1 w-56 bg-white rounded-lg shadow-lg border border-gray-200 z-20 text-gray-900">
                  <div className="px-4 py-3 border-b border-gray-100">
                    ...
                    <span className="inline-block mt-1 px-2 py-0.5 rounded bg-teal-100 text-teal-700 text-xs font-medium">Academic Advisor</span>
                  </div>
                  <button type="button" className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100 text-gray-700"
                    onClick={() => { setUserMenuOpen(false); router.push('/change-password'); }}>
                    Change Password
                  </button>
                  <hr className="my-1 border-gray-100" />
                  <button type="button" className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100 text-red-600" onClick={handleLogout}>
                    Logout
                  </button>
                </div>
              </>
      ...
      <div className="flex flex-1 min-h-0">
        <aside className={`shrink-0 border-r border-gray-200 bg-white flex flex-col transition-[width] duration-200 ${sidebarOpen ? 'w-56' : 'w-0 overflow-hidden'}`}
              ...
              <nav className="space-y-5 px-3">
                {navSections.map(section => (
                  <div key={section.label}>
                    <div className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest px-2 mb-1">{section.label}</div>
                    <div className="space-y-0.5">
                      {section.items.map(item => {
                        const isActive = pathname === item.href || (!exactOnlyPaths.includes(item.href) && pathname.startsWith(item.href + '/'));
                        return (
                          <Link key={item.href} href={item.href}
                            className={`flex items-center px-3 py-2 rounded-lg text-sm transition-colors ${isActive ? 'bg-teal-50 text-teal-700 font-medium border-l-[3px] border-teal-600' : 'text-gray-600 hover:bg-gray-50'}`}>
                            {item.label}
                          </Link>
                        );
                      })}
