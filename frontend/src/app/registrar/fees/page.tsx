...
function exportFeeAccountsCSV(accounts: FeeAccount[]) {
  const header = ['Student', 'Student No', 'Term', 'Total (ETB)', 'Paid (ETB)', 'Balance (ETB)', 'Due Date', 'Status'];
  ...
  a.href = url;
  a.download = `student-fee-accounts-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
        ...
        <div className="flex items-center gap-2">
          {filtered.length > 0 && (
            <div className="flex items-center gap-2 no-print">
              <button type="button" onClick={() => exportFeeAccountsCSV(filtered)}
          ...
          <button type="button" onClick={openCreateModal}
            className="inline-flex items-center gap-2 px-4 py-2 bg-purple-700 hover:bg-purple-800 text-white rounded-lg text-sm font-medium no-print">
