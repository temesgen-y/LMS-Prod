...
function exportStatementsCSV(statements: FeeStatement[]) {
  const header = ['Student', 'Student No', 'Term', 'Total (ETB)', 'Paid (ETB)', 'Balance (ETB)', 'Due Date', 'Status'];
  const dataRows = statements.map(s => [
    s.student_name,
    s.student_no,
    s.term_name,
  ...
  a.href = url;
  a.download = `fee-statements-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Fee Statements</h1>
        {filtered.length > 0 && (
          <div className="flex items-center gap-2 no-print">
            <button
              type="button"
              onClick={() => exportStatementsCSV(filtered)}
