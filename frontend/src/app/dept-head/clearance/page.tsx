...
export default function DeptHeadClearancePage() {
  ...
  // ── Toggle dept_cleared ─────────────────────────────────────────────────────
  const toggleDeptCleared = async (req: ClearanceRequest) => {
    setActionLoading(req.id);
    try {
      const supabase = createClient();
      const newDept = !req.dept_cleared;
      const allCleared = req.library_cleared && newDept && req.registrar_cleared;
      const newStatus = allCleared
        ? 'cleared'
        : (req.library_cleared || newDept || req.registrar_cleared) ? 'in_progress' : 'pending';

      await supabase.from('clearance_requests').update({
        dept_cleared: newDept,
        status:       newStatus,
  ...
  const saveEdit = async () => {
    ...
    try {
      const supabase = createClient();
      const allCleared = editTarget.library_cleared && editDept && editTarget.registrar_cleared;
      const derivedStatus = editStatus === 'rejected'
        ? 'rejected'
        : allCleared
          ? 'cleared'
          : (editTarget.library_cleared || editDept || editTarget.registrar_cleared) ? 'in_progress' : 'pending';
      ...
      if (derivedStatus !== editTarget.status) {
        const bodyMap: Record<string, string> = {
          ...
          rejected:    `Your ${editType} clearance request has been rejected by the department.`,
          in_progress: `Your ${editType} clearance is now in progress (department approved).`,
        };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await supabase.from('notifications').insert({
        ...
        body:    `Your ${deleteTarget.clearance_type} clearance request has been removed by the department.`,
        is_read: false,
      });
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Clearance Requests</h1>
      <p className="text-sm text-gray-500 mb-6">Manage department clearance approvals for students.</p>
                ...
                {filtered.map(r => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3 text-center">
                      <button
                        type="button"
                        onClick={() => toggleDeptCleared(r)}
                        disabled={actionLoading === r.id || r.status === 'cleared' || r.status === 'rejected'}
                        title={r.dept_cleared ? 'Click to un-clear dept' : 'Click to mark dept cleared'}
                      ...
                      >
                        {actionLoading === r.id ? '…' : r.dept_cleared ? '✓' : '○'}
                      </button>
                    </td>
                    <td className="px-5 py-3 text-center"><CheckIcon cleared={r.registrar_cleared} /></td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <div className="p-6 space-y-5">
              ...
              {/* Dept cleared toggle */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Department Clearance</label>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={editDept}
                    onChange={e => setEditDept(e.target.checked)}
                    className="w-4 h-4 rounded accent-purple-600"
                  />
                  <span className="text-sm text-gray-700">Department Cleared</span>
                </label>
                <p className="text-xs text-gray-400 mt-1">Library and Registrar clearances are managed by their respective offices.</p>
              </div>

              {/* Other clearances read-only */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Other Clearances (read-only)</label>
                <div className="flex gap-6">
                  <span className={`flex items-center gap-1.5 text-sm ${editTarget.library_cleared ? 'text-green-600' : 'text-gray-400'}`}>
                    {editTarget.library_cleared ? '✓' : '○'} Library
                  </span>
                  <span className={`flex items-center gap-1.5 text-sm ${editTarget.registrar_cleared ? 'text-green-600' : 'text-gray-400'}`}>
                    {editTarget.registrar_cleared ? '✓' : '○'} Registrar
                  </span>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">Status is auto-calculated from clearance flags unless set to Rejected.</p>
              </div>
