    ...
    <div className="flex items-end gap-3 justify-center">
      {d > 0 && (
        <div className="text-center">
          <p className="font-mono text-4xl font-bold text-gray-900">{d}</p>
          <p className="text-xs text-gray-500 mt-1">day{d !== 1 ? 's' : ''}</p>
        </div>
      )}
      <div className="text-center">
        <p className="font-mono text-4xl font-bold text-gray-900">{String(h).padStart(2, '0')}</p>
        <p className="text-xs text-gray-500 mt-1">hr</p>
      </div>
      <p className="font-mono text-4xl font-bold text-gray-400 mb-5">:</p>
      <div className="text-center">
        <p className="font-mono text-4xl font-bold text-gray-900">{String(m).padStart(2, '0')}</p>
        <p className="text-xs text-gray-500 mt-1">min</p>
      </div>
      <p className="font-mono text-4xl font-bold text-gray-400 mb-5">:</p>
      <div className="text-center">
        <p className="font-mono text-4xl font-bold text-gray-900">{String(s).padStart(2, '0')}</p>
...
export default function StudentVirtualClassroomPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data: enrollment } = await supabase
      ...
      .select('id')
      .eq('student_id', uid)
      .eq('offering_id', (raw as any).offering_id)
    ...
    const { data: instrUser } = await supabase
      ...
      .eq('id', (raw as any).instructor_id)
      .single();
    ...
    if (att) setMyAttendance({ joinedAt: (att as any).joined_at, leftAt: (att as any).left_at });
    setLoading(false);
  }, [sessionId, router]);
  ...
  const handleJoin = async () => {
    ...
    setMyAttendance({ joinedAt: now, leftAt: null });
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-5">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {isCompleted && (
                  <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-gray-100 text-gray-500">Completed</span>
                )}
                {isCancelled && (
                  <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-red-50 text-red-600">Cancelled</span>
                )}
              </div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                Instructor: {session.instructorName} · {PLATFORM_LABELS[session.platform] ?? session.platform} · {session.durationMins} min
              </p>
            </div>
          </div>
          ...
          <div className={`rounded-2xl border p-8 text-center space-y-4 ${isLive ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-200'}`}>
            {isLive ? (
              <>
                ...
                <p className="text-sm text-green-600">Your instructor has started the session. Join now to participate.</p>
              </>
            ) : (
              <>
                <p className="text-blue-700 font-semibold">Session starting soon</p>
                <p className="text-sm text-blue-600">You can join up to 15 minutes early.</p>
              </>
            )}
            ...
            {myAttendance?.joinedAt && (
              <p className="text-xs text-gray-500">
                You joined at {new Date(myAttendance.joinedAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
              </p>
            )}
            ...
            {session.meetingId && (
              <div className="flex items-center justify-center gap-4 text-xs text-gray-500 pt-2">
                ...
                {session.passcode && (
                  <span>Passcode: <span className="font-mono font-medium text-gray-700">{session.passcode}</span></span>
                )}
              </div>
            )}
          </div>
        ...
        {isCompleted && (
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 text-center space-y-4">
            ...
            {myAttendance?.joinedAt ? (
              ...
              <p className="text-sm text-gray-400">You did not attend this session.</p>
            )}
        ...
        {/* Connection info (always visible when not completed/cancelled) */}
