...
const initialAddForm = { firstName: '', lastName: '', email: '', staffNo: '', departmentId: '', password: '' };

export default function AdminStaffDeptHeadsPage() {
  ...
  // Edit modal
  const [editTarget, setEditTarget] = useState<DeptHead | null>(null);
  const [editForm, setEditForm] = useState({ firstName: '', lastName: '', staffNo: '', departmentId: '', status: 'active' });
  const [editSubmitting, setEditSubmitting] = useState(false);
  const [editError, setEditError] = useState('');

  // Deactivate confirm
  const [deactivateTarget, setDeactivateTarget] = useState<DeptHead | null>(null);
  ...
  const fetchDeptHeads = useCallback(async () => {
    setLoading(true);
    const supabase = createClient();
    ...
    if (error) {
      ...
      const rows: DeptHead[] = (data ?? []).map((row: any) => {
        const profile = Array.isArray(row.department_head_profiles)
          ? row.department_head_profiles[0]
          : row.department_head_profiles;
        return {
          userId: row.id,
  ...
  useEffect(() => {
    fetchDepartments();
    fetchDeptHeads();
  }, [fetchDepartments, fetchDeptHeads]);

  // Resolve department names once both are loaded
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!addForm.departmentId) { setAddError('Department is required.'); return; }
    ...
    try {
      const res = await fetch('/api/admin/staff/create', {
        ...
        body: JSON.stringify({
          ...
          department: addForm.departmentId,
        }),
      });
  ...
  const openEdit = (dh: DeptHead) => {
    const parts = dh.fullName.split(' ');
    setEditForm({
      firstName: parts[0] ?? '',
  ...
  const handleDeactivate = async () => {
    ...
    else { toast.success(`${deactivateTarget.fullName} deactivated.`); setDeactivateTarget(null); fetchDeptHeads(); }
  };

  const handleActivate = async (dh: DeptHead) => {
    const supabase = createClient();
    const { error } = await supabase
      .from('department_head_profiles')
      .update({ profile_status: 'active' })
      .eq('user_id', dh.userId);
    if (error) { toast.error(error.message); }
    else { toast.success(`${dh.fullName} activated.`); fetchDeptHeads(); }
  };

  const filtered = deptHeads.filter((d) =>
    d.fullName.toLowerCase().includes(search.toLowerCase()) ||
    d.email.toLowerCase().includes(search.toLowerCase()) ||
    d.staffNo.toLowerCase().includes(search.toLowerCase()) ||
    (deptMap[d.departmentId] ?? '').toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-stretch sm:items-center">
        <div className="relative flex-1 max-w-md">
          <input type="search" placeholder="Search department heads..."
        ...
        <>
          ...
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md max-h-[90vh] flex flex-col bg-white rounded-xl shadow-xl border border-gray-200"
            ...
            <div className="flex items-center justify-between shrink-0 p-6 pb-0">
              <h2 className="text-lg font-bold text-gray-900">Add Department Head Account</h2>
                ...
                <div className="w-full text-left space-y-3 bg-gray-50 rounded-lg p-4 text-sm">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <p className="font-medium text-gray-900 flex-1 font-mono">
                        {showPassword ? addSuccess.password : '••••••••••••'}
                      </p>
                      <button type="button" onClick={() => setShowPassword((s) => !s)}
                        className="text-xs text-primary hover:underline shrink-0">{showPassword ? 'Hide' : 'Show'}</button>
                      <button type="button"
                        onClick={() => { navigator.clipboard.writeText(addSuccess.password); toast.success('Password copied!'); }}
                        className="text-xs text-primary hover:underline shrink-0">Copy</button>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4 overflow-y-auto pr-1 max-h-[60vh]">
                  ...
                  <div>
                    ...
                    {departments.length === 0 ? (
                      <p className="text-sm text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                        ...
                        <a href="/admin/departments" className="underline font-medium" target="_blank" rel="noreferrer">Add departments first.</a>
                      </p>
                    ) : (
                      <select required value={addForm.departmentId}
                        onChange={(e) => setAddForm((f) => ({ ...f, departmentId: e.target.value }))} className={inputCls}>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      Staff Number <span className="text-gray-400 font-normal">(optional)</span>
                    </label>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">Share this with the department head. They can change it after login.</p>
      ...
      {editTarget && (
        <>
          ...
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md max-h-[90vh] flex flex-col bg-white rounded-xl shadow-xl border border-gray-200"
            role="dialog" aria-modal="true">
            <div className="flex items-center justify-between shrink-0 p-6 pb-0">
              <h2 className="text-lg font-bold text-gray-900">Edit Department Head</h2>
              ...
              <div className="space-y-4 overflow-y-auto pr-1 max-h-[60vh]">
                ...
                <div>
                  ...
                  <select required value={editForm.departmentId}
                    onChange={(e) => setEditForm((f) => ({ ...f, departmentId: e.target.value }))} className={inputCls}>
                    <option value="">Select department…</option>
                    {departments.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
                  </select>
                </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          <div className="px-5 py-16 text-center text-sm text-gray-500">Loading department heads...</div>
        ) : deptHeads.length === 0 ? (
          <div className="px-5 py-16 text-center">
            <p className="text-gray-600 font-medium mb-1">No department head accounts yet.</p>
            <p className="text-sm text-gray-400 mb-4">Create the first department head account.</p>
            <button type="button" onClick={openAdd}
              ...
              <table className="w-full min-w-[760px]">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50/80">
                    {['Name', 'Email', 'Staff No', 'Department', 'Status', 'Created', 'Actions'].map((h) => (
                ...
                <tbody>
                  {paginated.length === 0 ? (
                    <tr><td colSpan={7} className="px-5 py-10 text-center text-sm text-gray-500">No department heads match your search.</td></tr>
                  ) : paginated.map((dh) => (
                    <tr key={dh.userId} className="border-b border-gray-100 hover:bg-gray-50/50">
                      <td className="px-5 py-3 text-sm font-medium text-gray-900">{dh.fullName}</td>
                      <td className="px-5 py-3 text-sm text-gray-600">{dh.email}</td>
                      <td className="px-5 py-3 text-sm text-gray-600">{dh.staffNo || '—'}</td>
                      <td className="px-5 py-3 text-sm text-gray-600">{deptMap[dh.departmentId] ?? '—'}</td>
                      <td className="px-5 py-3">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          dh.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'
                        }`}>
                          {dh.status.charAt(0).toUpperCase() + dh.status.slice(1)}
                        </span>
                      </td>
                      <td className="px-5 py-3 text-sm text-gray-500">{fmtDate(dh.createdAt)}</td>
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          <button type="button" onClick={() => openEdit(dh)}
                            className="px-3 py-1 rounded text-xs font-medium bg-gray-100 text-gray-700 hover:bg-gray-200">Edit</button>
                          {dh.status === 'active' ? (
                            <button type="button" onClick={() => setDeactivateTarget(dh)}
                              className="px-3 py-1 rounded text-xs font-medium bg-red-50 text-red-700 hover:bg-red-100">Deactivate</button>
                          ) : (
                            <button type="button" onClick={() => handleActivate(dh)}
