...
const initialAddForm = { firstName: '', lastName: '', email: '', staffNo: '', accessLevel: 'standard', password: '' };

export default function AdminStaffITAdminsPage() {
  ...
  // Edit modal
  const [editTarget, setEditTarget] = useState<ITAdmin | null>(null);
  const [editForm, setEditForm] = useState({ firstName: '', lastName: '', staffNo: '', accessLevel: 'standard', status: 'active' });
  const [editSubmitting, setEditSubmitting] = useState(false);
  const [editError, setEditError] = useState('');

  // Deactivate confirm
  const [deactivateTarget, setDeactivateTarget] = useState<ITAdmin | null>(null);
  const [deactivating, setDeactivating] = useState(false);

  const fetchITAdmins = useCallback(async () => {
    setLoading(true);
    ...
    if (error) {
      toast.error('Failed to load IT administrators.');
    } else {
      const rows: ITAdmin[] = (data ?? []).map((row: any) => {
        const profile = Array.isArray(row.it_admin_profiles)
          ? row.it_admin_profiles[0]
          : row.it_admin_profiles;
        return {
          userId: row.id,
          ...
          accessLevel: profile?.access_level ?? 'standard',
  ...
  useEffect(() => { fetchITAdmins(); }, [fetchITAdmins]);
    ...
    try {
      const res = await fetch('/api/admin/staff/create', {
        ...
        body: JSON.stringify({
          ...
          access_level: addForm.accessLevel,
        }),
      });
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('IT administrator updated.');
      setEditTarget(null);
      fetchITAdmins();
    } catch (err) {
  ...
  const handleDeactivate = async () => {
    ...
    else { toast.success(`${deactivateTarget.fullName} deactivated.`); setDeactivateTarget(null); fetchITAdmins(); }
  };

  const handleActivate = async (admin: ITAdmin) => {
    const supabase = createClient();
    const { error } = await supabase
      .from('it_admin_profiles')
    ...
    if (error) { toast.error(error.message); }
    else { toast.success(`${admin.fullName} activated.`); fetchITAdmins(); }
  };

  const filtered = itAdmins.filter((a) =>
    a.fullName.toLowerCase().includes(search.toLowerCase()) ||
    a.email.toLowerCase().includes(search.toLowerCase()) ||
    a.staffNo.toLowerCase().includes(search.toLowerCase()) ||
    a.accessLevel.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-stretch sm:items-center">
        <div className="relative flex-1 max-w-md">
          <input type="search" placeholder="Search IT administrators..."
        ...
        <>
          ...
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md max-h-[90vh] flex flex-col bg-white rounded-xl shadow-xl border border-gray-200"
            ...
            <div className="flex items-center justify-between shrink-0 p-6 pb-0">
              <h2 className="text-lg font-bold text-gray-900">Add IT Administrator Account</h2>
              ...
              <form onSubmit={handleAdd} className="flex flex-col flex-1 min-h-0 p-6">
                <div className="space-y-4 overflow-y-auto pr-1 max-h-[60vh]">
                  ...
                  <div>
                    ...
                    <select value={addForm.accessLevel}
                      onChange={(e) => setAddForm((f) => ({ ...f, accessLevel: e.target.value }))} className={inputCls}>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">Share this with the IT administrator. They can change it after login.</p>
      ...
      {editTarget && (
        <>
          ...
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md max-h-[90vh] flex flex-col bg-white rounded-xl shadow-xl border border-gray-200"
            role="dialog" aria-modal="true">
            <div className="flex items-center justify-between shrink-0 p-6 pb-0">
              <h2 className="text-lg font-bold text-gray-900">Edit IT Administrator</h2>
            ...
            <form onSubmit={handleEdit} className="flex flex-col flex-1 min-h-0 p-6">
              <div className="space-y-4 overflow-y-auto pr-1 max-h-[60vh]">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Access Level</label>
                  <select value={editForm.accessLevel}
                    onChange={(e) => setEditForm((f) => ({ ...f, accessLevel: e.target.value }))} className={inputCls}>
                    <option value="standard">Standard</option>
                    <option value="super">Super</option>
                  </select>
                </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          <div className="px-5 py-16 text-center text-sm text-gray-500">Loading IT administrators...</div>
        ) : itAdmins.length === 0 ? (
          <div className="px-5 py-16 text-center">
            <p className="text-gray-600 font-medium mb-1">No IT administrator accounts yet.</p>
            <p className="text-sm text-gray-400 mb-4">Create the first IT administrator account.</p>
            <button type="button" onClick={openAdd}
              ...
              <table className="w-full min-w-[720px]">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50/80">
                    {['Name', 'Email', 'Staff No', 'Access Level', 'Status', 'Created', 'Actions'].map((h) => (
                ...
                <tbody>
                  {paginated.length === 0 ? (
                    <tr><td colSpan={7} className="px-5 py-10 text-center text-sm text-gray-500">No IT admins match your search.</td></tr>
                  ) : paginated.map((admin) => (
                    <tr key={admin.userId} className="border-b border-gray-100 hover:bg-gray-50/50">
                      <td className="px-5 py-3 text-sm font-medium text-gray-900">{admin.fullName}</td>
                      <td className="px-5 py-3 text-sm text-gray-600">{admin.email}</td>
                      <td className="px-5 py-3 text-sm text-gray-600">{admin.staffNo || '—'}</td>
                      <td className="px-5 py-3">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          admin.accessLevel === 'super' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'
                        }`}>
                          {admin.accessLevel.charAt(0).toUpperCase() + admin.accessLevel.slice(1)}
                        </span>
                      </td>
                      <td className="px-5 py-3">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      ...
                      <td className="px-5 py-3 text-sm text-gray-500">{fmtDate(admin.createdAt)}</td>
                      <td className="px-5 py-3">
