...
const initialAddForm = { firstName: '', lastName: '', email: '', staffNo: '', department: '', password: '' };

export default function AdminStaffRegistrarsPage() {
  ...
  // Edit modal
  const [editTarget, setEditTarget] = useState<Registrar | null>(null);
  const [editForm, setEditForm] = useState({ firstName: '', lastName: '', staffNo: '', department: '', status: 'active' });
  const [editSubmitting, setEditSubmitting] = useState(false);
  const [editError, setEditError] = useState('');

  // Deactivate confirm
  const [deactivateTarget, setDeactivateTarget] = useState<Registrar | null>(null);
  const [deactivating, setDeactivating] = useState(false);

  const fetchRegistrars = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/staff/registrars');
      const result = await res.json();
      if (!res.ok) {
        toast.error(result.error || 'Failed to load registrars.');
      } else {
        setRegistrars(result.registrars ?? []);
      }
    } catch {
  ...
  const handleDeactivate = async () => {
    ...
    else { toast.success(`${deactivateTarget.fullName} deactivated.`); setDeactivateTarget(null); fetchRegistrars(); }
  };

  const handleActivate = async (reg: Registrar) => {
    const supabase = createClient();
    const { error } = await supabase
      .from('registrar_profiles')
    ...
    if (error) { toast.error(error.message); }
    else { toast.success(`${reg.fullName} activated.`); fetchRegistrars(); }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <>
          ...
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md max-h-[90vh] flex flex-col bg-white rounded-xl shadow-xl border border-gray-200"
            ...
            <div className="flex items-center justify-between shrink-0 p-6 pb-0">
              <h2 className="text-lg font-bold text-gray-900">Add Registrar Account</h2>
            ...
            {addSuccess ? (
              ...
              <form onSubmit={handleAdd} className="flex flex-col flex-1 min-h-0 p-6">
                <div className="space-y-4 overflow-y-auto pr-1 max-h-[60vh]">
                  {addError && (
                    <div className="rounded-lg bg-red-50 border border-red-200 text-red-800 text-sm px-3 py-2">{addError}</div>
                  )}
                  ...
                  <div>
                    ...
                    <input type="text" value={addForm.staffNo}
                      onChange={(e) => setAddForm((f) => ({ ...f, staffNo: e.target.value }))}
                      className={inputCls} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                  ...
                  <div>
                    ...
                    <input type="password" required value={addForm.password}
                      ...
                      className={inputCls} />
                    <p className="text-xs text-gray-500 mt-1">Share this with the registrar. They can change it after login.</p>
      ...
      {editTarget && (
        <>
          ...
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md max-h-[90vh] flex flex-col bg-white rounded-xl shadow-xl border border-gray-200"
            role="dialog" aria-modal="true">
            <div className="flex items-center justify-between shrink-0 p-6 pb-0">
              <h2 className="text-lg font-bold text-gray-900">Edit Registrar</h2>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          <div className="px-5 py-16 text-center text-sm text-gray-500">Loading registrars...</div>
        ) : registrars.length === 0 ? (
          <div className="px-5 py-16 text-center">
            <p className="text-gray-600 font-medium mb-1">No registrar accounts yet.</p>
            <p className="text-sm text-gray-400">Create the first registrar account using the button above.</p>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[760px]">
                ...
                <tbody>
                  {paginated.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="px-5 py-10 text-center text-sm text-gray-500">No registrars match your search.</td>
                    </tr>
                  ) : paginated.map((reg) => (
                    <tr key={reg.userId} className="border-b border-gray-100 hover:bg-gray-50/50">
                      <td className="px-5 py-3 text-sm font-medium text-gray-900">{reg.fullName}</td>
                      <td className="px-5 py-3 text-sm text-gray-600">{reg.email}</td>
                      <td className="px-5 py-3 text-sm text-gray-600">{reg.staffNo || '—'}</td>
                      <td className="px-5 py-3 text-sm text-gray-600">{reg.department || '—'}</td>
                      ...
                      <td className="px-5 py-3 text-sm text-gray-500">{fmtDate(reg.createdAt)}</td>
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          <button type="button" onClick={() => openEdit(reg)}
                            className="px-3 py-1 rounded text-xs font-medium bg-gray-100 text-gray-700 hover:bg-gray-200">
                            Edit
                          </button>
                          {reg.status === 'active' ? (
                            <button type="button" onClick={() => setDeactivateTarget(reg)}
                              className="px-3 py-1 rounded text-xs font-medium bg-red-50 text-red-700 hover:bg-red-100">
                              Deactivate
                            </button>
                          ) : (
                            <button type="button" onClick={() => handleActivate(reg)}
