...
const ACTION_COLORS: Record<string, string> = {
  instructor_invited:   'bg-blue-100 text-blue-700',
  instructor_deleted:   'bg-red-100 text-red-700',
  student_approved:     'bg-green-100 text-green-700',
  student_rejected:     'bg-red-100 text-red-700',
  user_deleted:         'bg-red-100 text-red-700',
  role_changed:         'bg-purple-100 text-purple-700',
  password_changed:     'bg-amber-100 text-amber-700',
  exam_terminated:      'bg-orange-100 text-orange-700',
  backup_created:       'bg-emerald-100 text-emerald-700',
  system_settings_updated: 'bg-indigo-100 text-indigo-700',
  unauthorized_access_attempt: 'bg-red-200 text-red-800 font-bold',
};
...
export default function AuditLogsPage() {
  ...
  const load = useCallback(async () => {
    ...
    ((users ?? []) as { id: string; first_name: string; last_name: string }[])
      .forEach(u => { nameMap[u.id] = `${u.first_name ?? ''} ${u.last_name ?? ''}`.trim(); });
    ...
    <div className="p-6 space-y-6 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Immutable record of all sensitive system actions</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <input
            ...
            className="px-3 py-2 rounded-lg border border-gray-200 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 w-48"
          />
          <select
            value={actionFilter}
